...
```{r}
date_range <- which(idatex$disc_clean_date %in% as.Date(c("1997-12-16")))

ggplot(idatex, aes(x = disc_clean_date, fill = state)) + 
  geom_vline(xintercept = as.numeric(FiresClean$disc_clean_date[date_range]), color = "blue", alpha = .8, size = 2) + 
  geom_density(alpha = .65, color = 'white') + 
